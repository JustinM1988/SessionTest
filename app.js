// app.js — UMD globals; no placeholders

if (!window.arcgisRestAuth || !window.arcgisRestRequest) {
  alert("Missing vendor scripts. Put auth.umd.min.js and request.umd.min.js into /vendor/ then reload.");
}

const UserSession = window.arcgisRestAuth && window.arcgisRestAuth.UserSession;
const request = window.arcgisRestRequest && window.arcgisRestRequest.request;
...
async function whoAmI(session) {
  const url = `https://www.arcgis.com/sharing/rest/community/self`;
